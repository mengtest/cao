...
rwtNpcGroup[tBox.Empty]["Awards"]["Events"][1]["UserMsg"]["Content"] = nMessageId
...
rwtNpcGroup[tCollect.TongEmpy]["Awards"]["Events"][1]["UserMsg"]["Content"] = nMessageId
...
rwtNpcGroup[tCollect.GuaiEmpy1]["Awards"]["Events"][1]["UserMsg"]["Content"] = nMessageId
...
--[[
    CopyMap_ShaHaiYiJi2_Test可测试输出随机的结果
    随机
    CopyMap_ShaHaiYiJi2_Test随机共同怪物2的随机（30% 3只 70% 2只）
    CopyMap_ShaHaiYiJi2_BoxSuiJi 随机宝箱，
        50% 怪物1(40379)=1只 50% 怪物1=2只
        固定2空(40376)
        加入之前随机怪物2(40380)的结果
        剩下的是奖励宝箱(40368)

        特殊宝箱位置是固定的(40369 3160402)上方房间
        特殊宝箱位置是固定的(40377 3160401)下方房间
        加中间的宝箱(40370 3160403)
    CopyMap_ShaHaiYiJi2_ColSuiJi 随机采集物
        桶的随机
            桶与怪的空一共有4个是空的(60512)
            桶一共6个 - 随机的空的数量 = 奖励的数量(60511)
        怪的随机（怪采集后会与怪物战斗的，模型必定是该怪物；是空或奖励则3种模型随机）
            50%怪物1=1只 50%怪物1=2只(60516)
            加入之前随机怪物2的结果(60517~60519)
            桶与怪的空一共有4个，减去桶的空的=怪的空的(60507,60514,60515)
            总的减去上面采集所有的=怪的奖励(60504,60520,60521)

            因为怪采集，怪物做死亡动作，采集光效会在怪物脚上，所以怪物采集(无模型有采集光效+0触发范围的死亡模型)，并且是两个位置
            sPos["CollGuai"]记录怪物采集(无模型有采集光效)的位置，无需角度
            sPos["CollGuai_MX"]记录纯怪物模型的位置和角度
    -=====================
    传送门每个传送门传的位置是固定的
    tValveGenIdInfo表记录了传送机关genid对应的目的地位置
        传送门由机关+传送门光效
        data2记录是否第一次传送门是：播放对白(7140)
        ["ValveEnterFunc"]中data7设1
        ["ValveLeaveFunc"]中data7设0，只有data7=0是才能传送，避免两端不停的传送
    ======================
    最后一个任务是杀怪
        任务流程为采集桶怪或开宝箱都能任务目标+1
        达到25的时候完成任务目标，出现对白(任务目标2)，通关

        所以宝箱和采集的逻辑都会判断掩码Data3的次数，达到25，播通关对白 （tSHYJ_DialogAwards）
        然后如果最后一个采集物或宝箱是怪物的话，出现对白会被卡掉，导致任务中断
        所以怪物对应的宝箱采集没有这个逻辑，相应的在怪物死亡函数 CopyMap_ShaHaiYiJi2_MonsterDie 中判断data3次数
]]
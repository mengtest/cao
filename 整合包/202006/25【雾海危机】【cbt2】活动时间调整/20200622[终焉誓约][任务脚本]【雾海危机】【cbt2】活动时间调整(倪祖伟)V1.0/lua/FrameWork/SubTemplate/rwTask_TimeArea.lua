...
function rwTask_TimeArea_IsGetTime(Info)
    ...
    for _,v in ipairs(Info["ActivityTime"]) do
        ...
        if v["Type"] == "KaiFuDays" then
            local nowdays = rwGetServerOpenDays()
            if nowdays < v["start"] then
                return false
            end

            local allDay = v["ChiXu"]+v["JianGe"]
            local mod = math.fmod(nowdays-v["start"],allDay)

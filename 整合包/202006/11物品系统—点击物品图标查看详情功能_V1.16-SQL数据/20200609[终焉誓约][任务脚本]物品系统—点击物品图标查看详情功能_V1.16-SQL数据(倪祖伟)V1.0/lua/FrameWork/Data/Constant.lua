...
--全服事件判断时间(小于等于开服时间)(格式:时间戳)(2020-01-01)
SERVEREVENT_CHECKTIME = 1577808000
...
-----------------脚本具体任务类型，客户端显示【支线】-----------------
    ...
    POKER = 106,     --王牌通缉
    DREAM_PRE = 107,	--温丽梦境前置任务任务
    SEE_GHOST = 108,	--见鬼系列任务
    NPC_RANDOMEVENT = 109,  --NPC随机事件
    WuHaiWeiJi = 110,  --雾海危机
    MIBAOHEYUE = 111,  --秘宝合约
    LIANMENG = 112,  --联盟任务
}
...
CONST_CODE_MSG = {
    [CONST_CODE.Unknown] = 100004,
    [CONST_CODE.Error_ChkMixLevel] = 100005,   --不满足最低等级检查
    [CONST_CODE.Error_ChkMaxLevel] = 100006,   --不满足最高等级检查
    [CONST_CODE.Error_ChkMoney] = 100007,      --金币数量不足
    [CONST_CODE.Error_ChkEmoney] = 100008,     --代币数量不足
--    [CONST_CODE.Error_ChkTradMoney] = 10071,  --交易币数量不足
    [CONST_CODE.Error_ChkItemNum] = 100009,    --物品数量不足
    [CONST_CODE.Error_ChkMoneyLimit] = 100010,      --金币数量达上限
    [CONST_CODE.Error_ChkEmoneyLimit] = 100011,     --代币数量达上限
--    [CONST_CODE.Error_ChkTradMoneyLimit] = 10075,     --交易币数量达上限
    [CONST_CODE.Error_ChkExpLimit] = 100012,   --经验达上限
    [CONST_CODE.Error_ChkTeamExpLimit] = 100013, --战队经验达上限
    [CONST_CODE.Error_ChkPackSpace] = 100014,    --背包格子不足
    [CONST_CODE.Error_DelItem] = 100015,         --删除物品失败
    [CONST_CODE.Error_ChkItemLimit] = 100016,         --物品数量超出携带上限
    [CONST_CODE.Error_ChkPetPackSpace] = 100017,         --幻兽背包空间不足
    [CONST_CODE.Error_ChkFriendBoxSpace] = 100018,         --好友宝箱剩余空间不足
    [CONST_CODE.Error_ChkAllFightHeroIsFail] = 100019,     --出战英雄检测不满足
    [CONST_CODE.Error_ChkSysFuncUnlock] = 100020,                --系统功能解锁不满足
}
...
CONST_AWARD_MODE_TYPE = {
    NORMAL_AWARD  = 1, --正常奖励
    LEVELADAPT_AWARD = 2, --根据等级发奖
}
--玩家属性
CONST_USER_ATTR={
    USER_INT_MONEY = 1, --金币
...
CONST_USER_TIMESTYPE = {
    ...
    Activity_CopyMap_SHYJ = 65  --沙海遗迹活动副本
}
...
CONST_SYSTEM_FUNCTION = {
    ...
    WELFARE = 36,		--福利系统
    EXPRESS = 37,		--风迹速递跑环
	ZHOUHUIFUBEN = 38,	--周回复本  秘境探索
	POKER = 39,		--王牌通缉
	FIGHTINGPOWER = 40,		--战斗力显示功能
	AOZI = 41,			--奥兹家族
	MOFAYIHUI = 42,		--魔法议会
	QIJIXUEYUAN = 43,	--奇迹学院
	MILINGUODU = 44,	--密林国度 密林新语
	SHENGJIE = 45,		--圣界
	YINGHAOLINGYU = 46,	--英豪领域	
	STRENGTHEN = 47,	--冒险方略（提升实力）
    WENJUAN3 = 48,		--问卷3 
    RwardTask = 49,		--悬赏任务
    MercenaryTask = 50,	--委托任务
    PalyCat = 51,		--撸猫
    YONGZHESHOP = 52,		--勇者商店（红蓝特典的商店）
    WuHaiWeiJi_Liget = 55,		--雾海危机光效
    WENJUAN4 = 57,		--问卷4 
    AP = 58,		    --体力 
}
...
CONST_TEXT_CONCAT_TYPE = {
    ...
    HERO_NAME = 6,      --英雄名字
    USER_NAME = 7,      --玩家名字
    DYN_TEXT = 8,       --动态文字（包含数值和字符串）
    CLIENT_TEXT = 9,    --索引91$Dynamic_content的ID
}

CONST_TEXT_JSON_TYPE = {
    NORMAL = 1,     --普通  Text  TextParameters
    DIALOG = 2,     --对白   Id   TextParameters
    OPTION = 3,     --选项  optionText  TextParameters
}
...
CONST_MENUTYPE = {
    ...
    Mercenary = 400,      --佣兵界面
    Bag = 500,            --背包
    TranferTarget = 600,  --传送目标选择
    Lottery = 700,        --占卜界面
    Pet = 800,            --本命界面
    Arena = 900,          --竞技场  --已被删除
    ShareBoss = 1000,     --分享BOSS（讨伐领主）
    ---后续界面ID同客户端界面ID
    --NpcTalk = 100, --Npc对话    --1
    --Hero = 200, --英雄          --2
...
--    Shop_Hero = 1630, -- 英雄场商店
    ...
    MiBaoHeYue = 4500,--秘宝合约
    SnowGirl = 4600,    --三星推图玩法
    SuperGuide = 4700,  --最强指南
    Exchange = 4800,    --兑换界面
} 
--对话框NPC表情
CONST_NPC_FACE={
...
--14日循环目标
CONST_LOOP_ACTIVITY_EVENT_TYPE ={
    SHARE_BOSS = 1,     --讨伐领主
    AREA_BOX = 2,       --探索层宝箱
    FRIEND_BOX = 3,     --好友宝箱
    MERCENARY = 4,      --委托任务
    ZHOUHUIFUBEN = 5,   --秘境探索
    PATROL= 6,          --巡查任务
    DAILY_COPY_MAP = 7, --每日副本
    DREAM_LAND = 8,     --温丽梦境
    AREAMONSTER = 9,    --探索层怪物
    LOTTERY = 10,       --友达速递
    EMONEY_LOTTERY = 11,--艾恩速递
    PVP = 12,           --好友切磋
    SHARE_BOSS_EST = 13,--讨伐最高级领主
    FRIEND_BOX_EST = 14,--开启最高级好友宝箱
    AREA_BOXEST = 15,   --开启最高级探索层宝箱
    MULTI_COPYMAP = 16, --多人副本
}

--NPC随机事件对应模块接口枚举
CONST_NPCRANDOMEVENT_UPTYPE = {
    League = 1, --自律联盟等级（有-达到）（大于等于）
    LeagueLess = 2, --自律联盟等级（无）（小于等于）
    LeagueBetween = 3, --自律联盟等级（无）（大于等于X 小于等于Y）
    TaskFinish = 4, --任务完成（有）（有）
    AreaUnLock = 5, --城镇解锁（无）（有）
    ------合成类型------
    LeagueAndTaskFinish = 101, --自律联盟等级+任务完成
    PreFunc = 102,
    LeagueAndAreaUnLock = 103,
}

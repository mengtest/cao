...
---------
local optionShowType = "showType" --选项显示方式(0:右侧显示,1:中间显示)
...
function rwNpcTemplate:Process()    
    ...
    --点击NPC处理逻辑
    if rwtNpc[self._NpcTypeId]["ClickAward"] then
        rwBaseAward:create(rwtNpc[self._NpcTypeId]["ClickAward"]):Process()
    end
    ...
    if rwtTaskEndNpcInfo[self._NpcTypeId] then
        ...
        if nTaskNum == 1 then
            bShowFlag = false
            rwLinkTaskNpcDialog(self._FinishTaskList[1])
...
function rwNpcTemplate:NpcDialogOption()    
    ...
    if rwChkTable(rwtNpc[self._NpcTypeId]["Option"]) then
        tOptionInfo[optionShowType] = rwtNpc[self._NpcTypeId]["Option"]["ShowType"] or 0
...
function rwNpcTemplate:NpcTaskDialogue()
    ...
    if rwChkTable(rwtTask[nTaskId]["ActivityTime"]) then
        if not rwTask_TimeArea_IsGetTime(rwtTask[nTaskId]) then
            if rwtTask[nTaskId]["NoTimeDialogId"] then
                rwOpenNpcChatDialog(rwtTask[nTaskId]["NoTimeDialogId"],CONST_NPCCHAT_TYPE.TASK,nTaskId)
            end
            return
        end
    end
...
function rwNpcTemplate:NpcDialogue()
	...
	if rwChkTable(rwtNpc[self._NpcTypeId]["ChkEvent"]) then
        rwBaseAward:create(rwtNpc[self._NpcTypeId]["ChkEvent"]):Process()
        --因为前往监狱的船长的对白的播放有多种条件限制，选择在奖励模板中播放对白
        --所以这里return，避免播放后面的默认对白“你好”
        return
	end
    ...
    if sMark == "DialogId" then
        if rwChkTable(rwtNpc[self._NpcTypeId]["ChkUserTempData"]) then
			...
			local nIndex = rwtNpc[self._NpcTypeId]["ChkUserTempData"]["Index"] or CONST_TEMP_DATA.Data1
            if rwTempDataIsExist(nTempId,rwUserGetId()) and rwTempDataGetValue(nTempId,nIndex,rwUserGetId()) == nData then
        ...
        local tDialog = LeagueChatGetDialogId(self._NpcTypeId)		
...
function rwNpcDialogOption(tInfo)
    ...
    local tJsonTable = {}
    tJsonTable["data"] = {}
    tJsonTable[optionShowType] = tInfo["ShowType"] or 0
...
function rwServerDialogText(nDialogId,tOtherDialog)
    if not nDialogId then
        return
    end
    local tJsonTable = {}      
    if rwtServerDialog[nDialogId] and rwtServerDialog[nDialogId]["DialogEndInit"] and rwtServerDialog[nDialogId]["DialogEndInit"]["Events"] then
	...
	local tDialogInfo = {}
    if tOtherDialog then
		tDialogInfo = tOtherDialog
	else
		tDialogInfo = rwtServerDialog[nDialogId]["ServerDialog"]
	end
--	sFunc = sFunc or "" 
...
--    tJsonTable[m_hasMore] = tDialogInfo["HasMore"] or 0  --是否有下一句
    ...
    if tDialogInfo["Option"] then
        rwNpcDialogOption(tDialogInfo["Option"])
    end
end
...
function rwNpcSubDialog(nNpcTypeId,nOptionIndex,nSubDialogIndex)
    ...
    if rwChkTable(tDialogInfo["Option"]) then
        tOptionInfo[optionShowType] = tDialogInfo["Option"]["ShowType"] or 0

    ...
    if rwChkTable(rwtNpc[self._NpcTypeId]["Option"]) then
        while(rwtNpc[self._NpcTypeId]["Option"][nIndex]) do
            ...
            if rwChkFunc(rwtNpc[self._NpcTypeId]["Option"][nIndex]["Chk"]) then
                bNpcOptionShow = rwtNpc[self._NpcTypeId]["Option"][nIndex]["Chk"](self._NpcTypeId)
            end
			if rwtNpc[self._NpcTypeId]["Option"][nIndex]["OnlyChkDialog"] and rwtNpc[self._NpcTypeId]["Option"][nIndex]["OnlyChkDialog"] == 1 then
    ...
    if sMark == "DialogId" then
		...
		if rwChkTable(rwtNpc[self._NpcTypeId]["LeagueDialog"]) then
			...
			if nDistanceTime >= 21600 and nDistanceTime <= 72000 then
				tDialog = tDialogInfo["DayDialog"] 
			else
				tDialog = tDialogInfo["NightDialog"] 
			end
			if rwChkInt(tDialog) then
				rwOpenNpcChatDialog(tDialog,CONST_NPCCHAT_TYPE.NPC,self._NpcTypeId)
				self._DialogId = tDialog
				return
			elseif rwChkTable(tDialog) then

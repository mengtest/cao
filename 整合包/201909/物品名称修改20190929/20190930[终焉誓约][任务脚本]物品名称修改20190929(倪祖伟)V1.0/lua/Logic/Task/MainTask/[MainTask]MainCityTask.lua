...
--    --这里要不要用这随机函数呢？如果用了，那点击伦纳德的时候会不会判断两次呢？
...
rwtNpc[tNpc_Mercenary[5]]["Option"][1]["Title"]= tLuaText[LANGUAGE_CONFIG][40030] --[40030] ="舌尖上的联盟" ,
rwtNpc[tNpc_Mercenary[5]]["Option"][1]["State"] = 0
...
rwtNpc[tNpc_Mercenary[5]]["Option"][1]["Func"] = function(nNpcTypeId)
    ---根据数据库接取任务
    rwFoodTask_AcceptTaskByUserRecord()
end
--【舌尖上的联盟】接任务按钮结束
...
-----------------------------------福利博彩-----------------------------------------
local tAward_BoCaiFuLi_Gift = {}
tAward_BoCaiFuLi_Gift["Events"] = {}
tAward_BoCaiFuLi_Gift["Events"][1] = {}
tAward_BoCaiFuLi_Gift["Events"][1]["GetServerAward"] = {}
tAward_BoCaiFuLi_Gift["Events"][1]["GetServerAward"]["AwardType"] = CONST_AWARD_TYPE.AWARD
tAward_BoCaiFuLi_Gift["Events"][1]["GetServerAward"]["AwardId"] = 7000899
tAward_BoCaiFuLi_Gift["Events"][1]["GetServerAward"]["ActionType"] = CONST_ACTION_TYPE.TIPS
tAward_BoCaiFuLi_Gift["Events"][1]["GetServerAward"]["ActionId"] = 8000107


rwtNpcGroup[21044] = rwtNpcGroup[21044] or {}
rwtNpcGroup[21044]["Type"] = CONST_NPCGROUP_TYPE.SingleNpc
rwtNpcGroup[21044]["NpcId"]= 3941

rwtNpc[3941] = rwtNpc[3941] or {}
rwtNpc[3941]["DialogId"] = {60903,60904,60905}
rwtNpc[3941]["NotOpenDialogId"] = {60903,60904,60905}
rwtNpc[3941]["ChkOpen"] = true
rwtNpc[3941]["Option"] = {}

--领取前
rwtNpc[3941]["Option"][1] = {}
rwtNpc[3941]["Option"][1]["Title"]= tLuaText[LANGUAGE_CONFIG][40033]
rwtNpc[3941]["Option"][1]["State"] = 0
rwtNpc[3941]["Option"][1]["Chk"] = function(nNpcId)
	if not rwTempDataIsExist(1246) then
		return true
	end
	return false
end
rwtNpc[3941]["Option"][1]["Func"] = function(nNpcTypeId)

	if rwUserTempDataSet(1246, 1, CONST_TEMP_DATA.Data1, 0,true) then
		rwOpenNpcChatDialog(60909)
		return true
	end	
	return
end

rwtDialog[60909] = rwtDialog[60909] or {}
rwtDialog[60909]["DialogEndInit"] = {}
rwtDialog[60909]["DialogEndInit"]["Events"] = {}
rwtDialog[60909]["DialogEndInit"]["Events"][1] = {}
rwtDialog[60909]["DialogEndInit"]["Events"][1]["GetServerAward"] = {}
rwtDialog[60909]["DialogEndInit"]["Events"][1]["GetServerAward"]["AwardType"] = CONST_AWARD_TYPE.AWARD
rwtDialog[60909]["DialogEndInit"]["Events"][1]["GetServerAward"]["AwardId"] = 7000898
rwtDialog[60909]["DialogEndInit"]["Events"][1]["GetServerAward"]["ActionType"] = CONST_ACTION_TYPE.TREASURE_BOX
rwtDialog[60909]["DialogEndInit"]["Events"][1]["GetServerAward"]["ActionId"] = 60909

	
--领取过
rwtNpc[3941]["Option"][2] = {}
rwtNpc[3941]["Option"][2]["Title"]= tLuaText[LANGUAGE_CONFIG][40038]
rwtNpc[3941]["Option"][2]["State"] = 0
rwtNpc[3941]["Option"][2]["Chk"] = function(nNpcId)
	
	if not rwTempDataIsExist(1246) or rwTempDataGetValue(1246, CONST_TEMP_DATA.Data2) == 1 then
		return false
	end
	
	return true
end
rwtNpc[3941]["Option"][2]["Func"] = function(nNpcTypeId)

	if rwTempDataDayJudgment(1246) then
		if rwUserGetItemCount(8000107) >= 1 then
		
			if rwUserTempDataSet(1246, 1, CONST_TEMP_DATA.Data2) then
				rwItemDel(8000107,1)
				rwBaseAward:create(tAward_BoCaiFuLi_Gift):Process()
				return true
			end
		end
...
--选项2
rwtNpc[3941]["Option"][3] = {}
rwtNpc[3941]["Option"][3]["Title"]= tLuaText[LANGUAGE_CONFIG][40034]
rwtNpc[3941]["Option"][3]["State"] = 0
rwtNpc[3941]["Option"][3]["Chk"] = function(nNpcId)
    return true
end
rwtNpc[3941]["Option"][3]["Func"] = function(nNpcTypeId)
    rwOpenNpcChatDialog(60908)
    return
end
--------------------------------------------------------------------------------------------
...
rwtTask[tMainCityTaskId[1]]["UnLockSysId"] = {CONST_SYSTEM_FUNCTION.SKILL_UPLEV}  --技能升级
...
rwtTask[tMainCityTaskId[2]]["UnLockSysId"] = {CONST_SYSTEM_FUNCTION.PLAYER_TRAINING}  --玩家训练(战技研习)CONST_SYSTEM_FUNCTION.WENJUAN1,问卷1，

...
tValveGenIdInfo[3160219]["Pos"] = "-272.34,-21.10,-29.71"--"-311.03,-21.16,11.421"
...
tValveGenIdInfo[3160229]["Pos"] = "-234.63,-21.20,-13.57"--"-225.60,-21.16,-20.05"
...
--进入副本初始化随机分布
function CopyMap_ShaHaiYiJi2_InitRandom()
    ...
    --宝箱genid合集;奖励随机到的genid 加入，根据各自坐标合集随机分布
    local tGenIdBox = {}
    ...
    local nNumBoxMonster1 = 1
    local nNumColMonster1 = 1
    local nNumBoxMonster2 = 0
    local nNumColMonster2 = 0
    local BoxCol = 2
    ...
    --桶采集空
    local nRateColEmpty_tong = math.random(0,4)
    ...
    for i =1,BoxCol do
        ...
        if tTogether[rate] == tGenId.BoxMonster2_1 then
            table.insert(tGenIdBox["Monster2"],tGenId.BoxMonster2_1)
            nNumBoxMonster2 = nNumBoxMonster2+1
        else
            table.insert(tGenIdCol_monster["Monster2"],tTogether[rate])
            nNumColMonster2 = nNumColMonster2+1
        end
        table.remove(tTogether,rate)
    end
    --剩余奖励事件
    local nBoxAward = 6-2- nNumBoxMonster1 - nNumBoxMonster2
    if nBoxAward>=1 then
        for i = 1,nBoxAward do
            table.insert(tGenIdBox["Award"],tGenId.BoxAward[i])
        end
    end
    local nColAward_Guai = 10 - nNumColMonster1 - nNumColMonster2 - ColEmpty_Guai
    ...
    --=========rwPrintErrorLog
    rwPrintErrorLog("======box")
    for _,v in pairs(tGenIdBox) do
        rwPrintErrorLog("_".._)
        for j,k in pairs(v) do
            rwPrintErrorLog(k)
        end
    end
    rwPrintErrorLog("======CollMons")
    ...
    rwPrintErrorLog("======Coltong")
    for _,v in pairs(tGenIdCol_tong) do
        rwPrintErrorLog("_".._)
        for j,k in pairs(v) do
            rwPrintErrorLog(k)
        end
    ...
    rwPrintErrorLog("momom"..#tPosColtong)
    rwPrintErrorLog("momom "..math.random(1,#tPosColtong))
    for i,v in pairs(tGenIdCol_tong) do
        for j,k in pairs(v) do
            rwPrintErrorLog("hh"..#tPosColtong)
    ...
    --local tRotColguai = SysCopyTable(tRot.CollGuai)
    for i,v in pairs(tGenIdCol_monster["Monster1"]) do
        local rate = math.random(1,#tPosColguai)
        rwAddGenEvent(v,tPosColguai[rate])
        ...
        rwtCopyMapDynInitInfo[nCopyMap][nDynMapId]["CollectInfo"][v]=nGenMX
        nGenMX = nGenMX+1
    end
    for i,v in pairs(tGenIdCol_monster["Monster2"]) do
        local rate = math.random(1,#tPosColguai)
        rwAddGenEvent(v,tPosColguai[rate])
        table.remove(tPosColguai,rate)
        local nNpcGroupid = tCollectMonster2[v]
    ...
    for i,v in pairs(tGenIdCol_monster["Empy"]) do
        local rate = math.random(1,#tPosColguai)
        local rateNpc = math.random(1,3)
        rwAddGenEvent(v,tPosColguai[rate],tCollect["GuaiEmpy"..rateNpc])
        table.remove(tPosColguai,rate)
    ...
    for i,v in pairs(tGenIdCol_monster["Award"]) do
        local rate = math.random(1,#tPosColguai)
        local rateNpc = math.random(1,3)
        rwAddGenEvent(v,tPosColguai[rate],tCollect["GuaiAward"..rateNpc])
        table.remove(tPosColguai,rate)
...
rwtNpcGroup[tCollect.TongAward]["Awards"]["Events"][1]["GetServerAward"]["ActionType"] = CONST_ACTION_TYPE.TREASURE_BOX
...
rwtNpcGroup[tCollect.GuaiAward1]["Awards"]["Events"][1]["GetServerAward"]["ActionType"] = CONST_ACTION_TYPE.TREASURE_BOX
...
rwtNpcGroup[tCollect.GuaiAward2]["Awards"]["Events"][1]["GetServerAward"]["ActionType"] = CONST_ACTION_TYPE.TREASURE_BOX
...
rwtNpcGroup[tCollect.GuaiAward3]["Awards"]["Events"][1]["GetServerAward"]["ActionType"] = CONST_ACTION_TYPE.TREASURE_BOX
...
rwtCopyMapMission[nCopyMap]["MapInitFunc"] = function (MissionId,MapId)
    ...
    CopyMap_ShaHaiYiJi2_InitRandom()
    rwPrintErrorLog("===============init end")
end

function moqi_shahai(index)
    if index == 99 then
        for i = 3160101, 3160110 do
            rwDelGenEvent(i)
            --rwSpecialStatus_RemoveEffect(7,0,2043)
        end
    end
    local tII = {}
    tII[1] = tCollect.GuaiMonster1
    tII[2] = tCollect.GuaiMonster2
    tII[3] = tCollect.GuaiMonster3
    tII[4] = tCollect.GuaiMonster4
    tII[5] = tCollect.GuaiEmpy1   
    tII[6] = tCollect.GuaiEmpy2   
    tII[7] = tCollect.GuaiEmpy3   
    tII[8] = tCollect.GuaiAward1  
    tII[9] = tCollect.GuaiAward2  
    tII[10]= tCollect.GuaiAward3  
    local gen = 3160101
    local index = index or 1
    for i,v in pairs(sPos["CollGuai"])do
        local mx = CollectMoxing[tII[index]]
        rwAddGenEvent(gen,v,mx,tRot.CollGuai[i])
        rwSpecialStatus_AddEffect(7,0,2043,v)
        gen =gen+1
    end
end
function moqi_shahai2(index)
    if index == 99 then
        for i = 3160101, 3160110 do
            rwDelGenEvent(i)
            --rwSpecialStatus_RemoveEffect(7,0,2043)
    ...
    for i,v in pairs(sPos["CollGuai_MX"]) do
        for k,j in ipairs(v) do
            rwAddGenEvent(gen,j,i,tRot.CollGuai_MX[i][k])
            gen =gen+1
        end
    end
end
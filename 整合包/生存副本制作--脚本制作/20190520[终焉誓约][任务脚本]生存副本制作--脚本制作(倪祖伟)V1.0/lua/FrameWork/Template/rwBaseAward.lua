...
-- 生存副本创建采集物事件
function rwBaseAward:CreateSurvivalCollect(tEventInfo)
    if rwChkTable(tEventInfo.CreateSurvivalCollect) then
        for _,Info in ipairs(tEventInfo.CreateSurvivalCollect) do

            local nNpcGroupId = Info.NPCGroupId
            local nMapId = Info.MapId
            local tPosInfo = Info["tPosInfo"]
            for i,v in pairs(Info.tGenIds) do
                if not rwHasGenEvent(v) then
                    rwSurvivalCopyMap_Create(nNpcGroupId,v,nMapId,tPosInfo)
                end
            end
        end
    end
...
function rwBaseAward:GetAward(tEventInfo)
	...
	self:CreateSurvivalCollect(tEventInfo)
end
...
--概率事件
function rwBaseAward:RandomToNormalEvents()

...
function rwLinkTaskNpcDialog(nTaskId)
    ...
    if rwChkInt(rwtTask[nTaskId]["DialogId"]) then
        rwOpenNpcChatDialog(rwtTask[nTaskId]["DialogId"],CONST_NPCCHAT_TYPE.TASK,nTaskId)
    elseif rwChkTable(rwtTask[nTaskId]["DialogId"]) then
...
function rwLinkNpcChatEnd(nIndex,nType,nId)
	...
	if nType == CONST_NPCCHAT_TYPE.TASK then
		...
		if nTaskDetailType and ENUM_TASK_TEMPLATE[nTaskDetailType] and rwtTask[nId] then
			if rwChkInt(rwtTask[nId]["DialogId"]) and nIndex == rwtTask[nId]["DialogId"] then
                ENUM_TASK_TEMPLATE[nTaskDetailType]:create(nId):DialogEndCallBack(nIndex)
            elseif rwChkTable(rwtTask[nId]["DialogId"]) then
                for _,v in pairs (rwtTask[nId]["DialogId"]) do
                    if nIndex == v then
                        ENUM_TASK_TEMPLATE[nTaskDetailType]:create(nId):DialogEndCallBack(nIndex)
                    end
                end
            elseif rwtTask[nId]["FinishDialogId"]  then
                if rwChkInt(rwtTask[nId]["FinishDialogId"]) and nIndex == rwtTask[nId]["FinishDialogId"] then
                    ENUM_TASK_TEMPLATE[nTaskDetailType]:create(nId):FinishDialogCallBack(nIndex)
                elseif rwChkTable(rwtTask[nId]["FinishDialogId"]) then
                    for _,v in pairs (rwtTask[nId]["FinishDialogId"]) do
                        if nIndex == v then
                            ENUM_TASK_TEMPLATE[nTaskDetailType]:create(nId):FinishDialogCallBack(nIndex)
                        end
                    end
            ...
            if rwChkInt(rwtTask[nId]["IsMercenaryBattleDialog"]) and rwtTask[nId]["IsMercenaryBattleDialog"] == nIndex then
                rwBaseTask:create(nId):FinishTask_OpenMercenaryMenu()
            end
		end
	else
	...
	--同位副本打开
	if nType == CONST_NPCCHAT_TYPE.SIDECOPYMAP then
		rwCloseLocalDialog(CONST_MENUTYPE.NpcTalk,nIndex)
		rwLinkCopyMapMissionEnter(nId)
	end
...
function rwLinkDialogOptionClick(nIndex,nType,nId,nOptionIndex)
    ...
    if nType == CONST_NPCCHAT_TYPE.TASK then
        ...
        if nTaskDetailType and ENUM_TASK_TEMPLATE[nTaskDetailType] then
            ENUM_TASK_TEMPLATE[nTaskDetailType]:create(nId):OnTaskDialogClick(nOptionIndex,nIndex)
        end
    else
...
--NPC选项点击
--nOptionIndex:选项索引
--nSubDialogIndex:子对话框索引
--nSubOptionIndex:子对话框选项索引
function rwLinkNpcOptionClick(nNpcId,nOptionIndex,nDialogId,nSubDialogIndex,nSubOptionIndex)

    --rwPrintWarnLog("------------------------------------")
    nDialogId = nDialogId or 0

    --下面2个参数用于子对话框
    nSubDialogIndex = nSubDialogIndex or 0
    nSubOptionIndex = nSubOptionIndex or 1
    ...
    local tInfo
    if nSubDialogIndex == 0 then
        tInfo = rwtNpc[nNpcId]["Option"][nOptionIndex] or {}
    else
        if rwChkTable(rwtNpc[nNpcId]["Option"][nOptionIndex]["SubDialog"]) then
            tInfo = rwtNpc[nNpcId]["Option"][nOptionIndex]["SubDialog"][nSubDialogIndex]["Option"][nSubOptionIndex]
        end
    end

    --默认关闭对白界面
    if not tInfo["IsNotCloseDialog"] then
        rwCloseLocalDialog(CONST_MENUTYPE.NpcTalk,nDialogId)
    end

    --额外接口
    if rwChkFunc(tInfo["Func"]) then
        tInfo["Func"](nNpcId)
    end

    if rwChkTable(tInfo["Node"]) then
        for _,v in pairs(tInfo["Node"]) do
            local bCondition = true
            if v["Condition"] then
                if not rwBaseAward:create(v["Condition"]):ChkCondition() then
                    bCondition = false
                end
            end
            if bCondition then
                if v["TurnSubIndex"] then
                    rwNpcSubDialog(nNpcId,nOptionIndex,v["TurnSubIndex"])
                end
            end
        end
    end
